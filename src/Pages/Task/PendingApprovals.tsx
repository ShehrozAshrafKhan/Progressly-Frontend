...
const rowsPerPage = 10;
  useEffect(() => {
    handleGetIsCompletedTasks();
  }, []);

  const handleGetIsCompletedTasks = async () => {
    try {
      const url = `${config.baseUrl}Tasks/GetIsCompletedTasks`;
  ...
  return (
    <Layout>
      <div className="container-fluid p-0">
        ...
        <div>
          <h2 className="mb-1 fw-bold text-dark">Pending Approvals</h2>
          <p className="text-muted mb-0">Review tasks that are awaiting approval.</p>
        </div>
      </div>
                ...
                {filteredTasks.length === 0 ? (
                  <tr>
                    <td colSpan={9} className="text-center py-5 text-muted">
                      <div className="d-flex flex-column align-items-center">
                        <i className="bi bi-clock-history fs-1 mb-2 text-light"></i>
                  ...
                  paginatedTasks.map((item, index) => (
                    <tr key={item.taskId || index} className="transition-base">
                      ...
                      <td className="px-4 py-3">
                         <span className="badge bg-warning bg-opacity-10 text-warning px-2 py-1 rounded-pill fw-medium small text-dark">
